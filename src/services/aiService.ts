...
// 提取 AI 文本
const getAiText = (response: any): string => {
    return (
...
请严格只返回 JSON，不要返回 markdown，不要加 \`\`\`json。
请按照以下 JSON 格式返回菜谱：
{
  "name": "菜品名称",
  ...
  "tips": ["实用技巧1：具体的操作要点", "注意事项2：避免常见错误的方法"]
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位经验丰富的专业厨师，擅长制作各种菜系的美食。请根据用户提供的食材和菜系要求，生成详细实用的菜谱。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const aiResponse = getAiText(response)
        console.log('AI raw response =>', aiResponse)
        console.log('AI raw response length =>', aiResponse?.length)

        const cleanResponse = cleanJsonText(aiResponse)
        console.log('AI clean response =>', cleanResponse)

        if (!cleanResponse) {
            throw new Error('模型返回内容为空，请检查模型是否支持当前请求格式')
        }

        const recipeData = safeJsonParse<any>(cleanResponse, {
            name: `${cuisine.name}推荐菜品`,
            ingredients,
            steps: [
                {
                    step: 1,
                    description: '当前模型返回的 JSON 格式不稳定，已使用兜底菜谱。',
                    time: 5,
                    temperature: '中火'
                }
            ],
            cookingTime: 20,
            difficulty: 'medium',
            tips: ['模型返回异常，已使用兜底菜谱', '建议稍后重试或更换模型']
        })
        ...
        const recipe: Recipe = {
            ...
            steps: recipeData.steps || [
                { step: 1, description: '准备所有食材', time: 5, temperature: '中火' },
                { step: 2, description: '按照传统做法烹饪', time: 20, temperature: '中火' }
            ],
        ...
        if (status === 401) {
            throw new Error(apiMessage || 'API Key 无效或未授权')
        }

        if (status === 404) {
            throw new Error(apiMessage || 'API 地址或模型名称不正确')
        }

        if (status === 429) {
            throw new Error(apiMessage || '请求过多或账户额度不足')
        }

        if (status === 400) {
            throw new Error(apiMessage || `${cuisine.name}表示这个食材搭配太有挑战性了`)
        }

        throw new Error(error instanceof Error ? error.message : `${cuisine.name}生成失败`)
    }
}
  ...
  "dishes": [
    {
      ...
      "tags": ["标签1", "标签2", "标签3"]
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位专业的菜单设计师，擅长根据不同场景和需求搭配合理的菜品组合。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const cleanResponse = cleanJsonText(aiResponse)
        const menuData = safeJsonParse<any>(cleanResponse, { dishes: [] })
...
请按照以下 JSON 格式返回菜谱：
{
  ...
  "tips": ["实用技巧：具体操作要点", "注意事项：避免常见问题"]
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位经验丰富的专业厨师，精通各种菜品的制作工艺。请根据菜品信息生成详细实用的制作菜谱，让家庭厨师也能轻松掌握。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const recipe: Recipe = {
            ...
            steps: recipeData.steps || [
                { step: 1, description: '准备食材', time: 5, temperature: '中火' },
                { step: 2, description: '开始制作', time: 15, temperature: '中火' }
            ],
        ...
        throw new Error(error instanceof Error ? error.message : '大厨表示这道菜太有挑战性了，请稍后重试')
    }
}
...
请按照以下 JSON 格式返回菜谱：
{
  ...
  "tips": ["实用技巧：具体操作要点", "注意事项：避免失败的关键"]
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位经验丰富的专业厨师，擅长根据用户需求定制菜谱。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const recipe: Recipe = {
            ...
            steps: recipeData.steps || [
                { step: 1, description: '准备所有食材', time: 5, temperature: '中火' },
                { step: 2, description: '按照要求烹饪', time: 20, temperature: '中火' }
            ],
            cookingTime: recipeData.cookingTime || 25,
            difficulty: recipeData.difficulty || 'medium',
        ...
        throw new Error(error instanceof Error ? error.message : '大厨表示这个自定义要求太有挑战性了，请稍后重试')
    }
}
...
export const generateMultipleRecipesStream = async (
    ...
    for (let index = 0; index < cuisines.length; index++) {
        ...
        try {
            ...
            if (onRecipeError) {
                const realError = error instanceof Error ? error : new Error(`${cuisine.name}生成失败`)
                onRecipeError(realError, index, cuisine, total)
            }
...
请按照以下 JSON 格式返回营养分析：
{
  ...
  "servingSize": "1人份"
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位专业的营养师，请根据菜谱信息生成详细的营养分析。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const cleanResponse = cleanJsonText(aiResponse)
        const nutritionData = safeJsonParse<any>(
            cleanResponse,
            generateFallbackNutrition(recipe.ingredients)
        )
...
请按照以下 JSON 格式返回一个最佳饮品搭配建议：
{
  ...
  "origin": "品牌或产地（可选）"
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位专业的饮品搭配师，请根据菜谱信息推荐合适的饮品搭配。优先推荐接地气的常见饮料，如可乐、雪碧、果汁、茶饮等，让普通家庭也能轻松享用。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const cleanResponse = cleanJsonText(aiResponse)
        const wineData = safeJsonParse<any>(
            cleanResponse,
            generateFallbackWinePairing({ id: 'custom', name: recipe.cuisine } as CuisineType, recipe.ingredients)
        )
...
请按照以下 JSON 格式返回菜谱：
{
  ...
  "tips": ["实用技巧1：具体的操作要点和成功秘诀", "注意事项2：避免常见错误的关键方法"]
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位经验丰富的中华料理大师，精通各种菜系的制作方法和传统工艺。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const recipe: Recipe = {
            ...
            steps: recipeData.steps || [
                { step: 1, description: '准备所有食材', time: 5, temperature: '中火' },
                { step: 2, description: '按照传统方法制作', time: 20, temperature: '中火' }
            ],
            cookingTime: recipeData.cookingTime || 25,
            difficulty: recipeData.difficulty || 'medium',
            tips: recipeData.tips || ['注意火候控制', '调味要适中'],
        ...
        throw new Error(error instanceof Error ? error.message : `大厨表示"${dishName}"这道菜太有挑战性了，请稍后重试`)
    }
}
...
请按照以下 JSON 格式返回酱料配方：
{
  ...
  "description": "酱料特色描述"
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位专业的酱料制作大师，精通各种传统和创新酱料的制作方法。请根据用户提供的酱料名称，生成详细、实用的制作教程。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const cleanResponse = cleanJsonText(aiResponse)
        const sauceData = safeJsonParse<any>(cleanResponse, {
            name: sauceName,
            category: 'complex',
            ingredients: ['主要食材', '调料'],
            steps: [
                { step: 1, description: '准备所有食材', time: 5, temperature: '中火', technique: '准备' },
                { step: 2, description: '按照传统方法制作', time: 20, temperature: '中火', technique: '制作' }
            ],
            makingTime: 25,
            difficulty: 'medium',
            tips: ['注意火候控制', '调味要适中'],
...
请按照以下 JSON 格式返回推荐结果：
{
  "recommendations": [
    ...
    "酱料名称3"
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位专业的酱料推荐专家，能够根据用户的口味偏好和使用场景推荐最合适的酱料。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const cleanResponse = cleanJsonText(aiResponse)
        const recommendationData = safeJsonParse<any>(cleanResponse, {
            recommendations: []
        })
...
请按照以下 JSON 格式返回酱料配方：
{
  ...
  "description": "创新酱料特色描述"
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位富有创意的酱料创作大师，擅长根据用户需求创作独特的酱料配方。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const sauceData = safeJsonParse<any>(cleanResponse, {
            ...
            ingredients: ['创新食材'],
            steps: [
                { step: 1, description: '准备创新食材', time: 5, temperature: '中火', technique: '准备' },
                { step: 2, description: '按照创意方法制作', time: 20, temperature: '中火', technique: '制作' }
            ],
            makingTime: 25,
            difficulty: 'medium',
            tips: ['发挥创意', '调整口味'],
...
请按照以下 JSON 格式返回搭配建议：
{
  "pairings": [
    ...
    "搭配菜品3 - 使用方法"
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位专业的美食搭配专家，精通各种酱料与菜品的最佳搭配方法。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const cleanResponse = cleanJsonText(aiResponse)
        const pairingData = safeJsonParse<any>(cleanResponse, {
            pairings: ['面条 - 拌面使用', '蔬菜 - 蘸食使用', '肉类 - 调味使用']
        })

        return pairingData.pairings || ['面条 - 拌面使用', '蔬菜 - 蘸食使用', '肉类 - 调味使用']
    } catch (error) {
...
请按照以下 JSON 格式返回占卜结果：
{
  ...
  "steps": ["制作步骤1", "制作步骤2"]
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位神秘而智慧的料理占卜师，精通星座学、生肖学和美食文化。你的话语充满神秘色彩，善于将占卜元素与美食完美结合。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
        ...
        const aiResponse = getAiText(response)
        const cleanResponse = cleanJsonText(aiResponse)
        const fortuneData = safeJsonParse<any>(cleanResponse, {
            dishName: '幸运料理',
            reason: '星座与生肖的神秘指引',
            luckyIndex: 8,
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位温暖而智慧的情感治愈师，深谙美食与情感的关系。你善于通过菜品来抚慰人心，话语温暖治愈。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位精通人际关系和美食文化的占卜师，善于分析人与人之间的默契和缘分。你的话语充满智慧和温暖。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        '你是一位精通数字学和美食文化的神秘占卜师，善于解读数字的深层含义并与菜品联系。你的话语充满神秘和智慧。请严格只返回 JSON 对象，不要包含任何额外文字，不要使用 markdown 代码块。请务必用中文回答。'
                },
                {
                    role: 'user',
                    content: prompt
