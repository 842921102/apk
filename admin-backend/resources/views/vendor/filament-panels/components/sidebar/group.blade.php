            ...
            @if ($collapsible)
                x-on:click="
                    (() => {
                        ...
                        const group = $event.currentTarget.closest('.fi-sidebar-group');
                        const nav = group?.closest('.fi-sidebar-nav-groups');

                        if (! nav) {
                            return;
                        }

                        const allLabels = [...nav.querySelectorAll('.fi-sidebar-group[data-group-label]')]
                            .map((el) => el.dataset.groupLabel)
                            .filter((l) => l);

                        $store.sidebar.collapsedGroups = allLabels.filter((l) => l !== label);

                        for (const li of group.querySelectorAll(
                            '.fi-sidebar-group-items > li.fi-sidebar-item',
                        )) {
                            const direct = li.querySelector(':scope > a.fi-sidebar-item-btn');
                            let href = direct?.getAttribute('href');
                            ...
                            const sub = li.querySelector(
                                '.fi-sidebar-sub-group-items a.fi-sidebar-item-btn[href]',
                            );
