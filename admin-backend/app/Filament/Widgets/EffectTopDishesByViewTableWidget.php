...
class EffectTopDishesByViewTableWidget extends TableWidget
{
        ...
        return $table
            ...
            ->records(function (): Collection {
                ...
                return collect($list)->values()->map(fn (array $row, int $i): array => [
                    ...$row,
                    ArrayRecord::getKeyName() => 'view-dish-'.$i.'-'.($row['dish'] ?? ''),
                ]);
            })
            ->columns([
                TextColumn::make('dish')->label('菜名')->wrap(),
                TextColumn::make('count')->label('曝光次数')->numeric()->alignEnd(),
            ])
            ->paginated(false);
    }
}
